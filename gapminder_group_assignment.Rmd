...
qplot(gdpPercap,lifeExp,data=df, color = year, xlab="Per-Capita GDP (int. dollars)", ylab="Life Expectancy (years)") + scale_x_log10()
```
...
ggplot(gapminder,
  ...
  geom_point(lwd = 1, show.legend = FALSE)  +  geom_smooth(method="lm") + facet_wrap(~ continent)
  scale_color_manual(values = continent_colors) +
  theme_bw() + theme(strip.text = element_text(size = rel(1.1)))
